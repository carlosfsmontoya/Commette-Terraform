...
resource "azurerm_app_service" "webapp3" {
  name                = "go-${var.project}-${var.environment}"
  location            = var.location
  resource_group_name = azurerm_resource_group.rg.name
  app_service_plan_id = azurerm_app_service_plan.app_service_plan.id

  site_config {
    linux_fx_version = "DOCKER|${azurerm_container_registry.acr.login_server}/${var.project}/go:latest"
    always_on        = true
    vnet_route_all_enabled = true
  }
...
resource "azurerm_app_service_virtual_network_swift_connection" "webapp3_vnet_integration" {
  app_service_id    = azurerm_app_service.webapp3.id
  subnet_id         = azurerm_subnet.subnetweb.id

  depends_on = [
    azurerm_app_service.webapp3
  ]
}